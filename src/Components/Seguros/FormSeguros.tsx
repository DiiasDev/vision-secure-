...
import { secoesSeguros } from './FieldsSeguros';

export default function FormSeguros() {
    const [inicioVigencia, setInicioVigencia] = useState<Dayjs | null>(null);
    const [fimVigencia, setFimVigencia] = useState<Dayjs | null>(null);
    ...
    const handleSubmit = (e: FormEvent<HTMLFormElement>) => {
        ...
        // Adicionar datas ao objeto
        if (inicioVigencia) {
            formObject.inicioVigencia = inicioVigencia.format('YYYY-MM-DD');
        }
        if (fimVigencia) {
            formObject.fimVigencia = fimVigencia.format('YYYY-MM-DD');
        }
        
        console.log('Dados do Seguro:', formObject);
                ...
                <div className="w-full max-w-7xl mx-auto p-8">
                    <div className="bg-[var(--bg-card)] rounded-2xl p-8 shadow-[var(--shadow-md)] border border-[var(--border-default)]">
                        ...
                        <p className="text-base text-[var(--text-secondary)] mb-8 leading-relaxed">
                            Preencha os dados do seguro para cadastro no sistema
                        </p>
                        
                        <form onSubmit={handleSubmit} className="space-y-8">
                            {secoesSeguros.map((secao, index) => (
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                        {secao.fields.map((campo) => (
                                            ...
                                            >
                                                ...
                                                {campo.type === 'date' ? (
                                                    <DatePicker
                                                        value={campo.name === 'inicioVigencia' ? inicioVigencia : fimVigencia}
                                                        onChange={(newValue) => {
                                                            if (campo.name === 'inicioVigencia') {
                                                                setInicioVigencia(newValue);
                                                            } else {
                                                                setFimVigencia(newValue);
                                                            }
                                                        }}
                                                        format="DD/MM/YYYY"
                                                        slotProps={{
                                                    ...
                                                    <select
                                                        ...
                                                        disabled={campo.isAsync && (!campo.options || campo.options.length === 0)}
                                                    >
                                                        <option value="">
                                                            {campo.isAsync ? 'Carregando opções...' : 'Selecione...'}
                                                        </option>
